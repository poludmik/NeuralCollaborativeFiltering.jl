...
function train_model(df_train::DataFrame, df_test::DataFrame, m::DotProductNCFModel;
                    bs=512,
                    lr = 0.015,
                    n_epochs = 102
                    )
    ...
    jldsave("weights\\dot_product_ncf\\model_dim$(m.emb_size)_bs$(bs)_ep$(n_epochs)_lr$(lr).jld2", model_state = Flux.state(m.model), test_loss = loss_all(test_data, m.model))
